...
cublas_path = "/home/facit/Faizan/reid/task_feb/StrongSORT-YOLO/reid/lib/python3.8/site-packages/nvidia/cublas/lib/:$LD_LIBRARY_PATH"
#---------------------------------------------------------------
#run bash command to set torch issue

os.environ['LD_LIBRARY_PATH'] = cublas_path
...
try:
    from yolov5.yutils.dataloaders import VID_FORMATS, LoadImages, LoadStreams
except:
    import sys
    sys.path.append('yolov5/utils')
    from dataloaders import VID_FORMATS, LoadImages, LoadStreams
from yolov5.yutils.general import (LOGGER, check_img_size, non_max_suppression, scale_coords, check_requirements, cv2,
                                  check_imshow, xyxy2xywh, increment_path, strip_optimizer, colorstr, print_args, check_file)
from yolov5.yutils.torch_utils import select_device, time_sync
from yolov5.yutils.plots import Annotator, colors, save_one_box
from strong_sort.ss_utils.parser import get_config
...
def run(
        ...
        draw=False,
        save_tracklets=False,  # draw object trajectory lines
        compare = False,
        use_ctl = False
    ...
    trajectory = {}
    #storing tracklets data for reid
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        #faizan: added code to keep track of frame ids
        frame_ids = []
        t1 = time_sync()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                    ...
                    if len(outputs[i]) > 0:
                        for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                            ...
                            if type(appearance) != list and type(appearance) == np.ndarray:
                                ...
                                # tracklet["appearance"] =  appearance.flatten().tolist()
                            else:
                            ...
                            if save_vid or save_crop or show_vid:  # Add bbox to image
                                ...
                                # x, y, w, h = bboxes
                                # roi = im0[y:y+h, x:x+w]
                                # blurred_roi = cv2.GaussianBlur(roi, (25, 25), sigmaX=10)
                                # im0[y:y+h, x:x+w] = blurred_roi
                ...
                #Faizan: wrote code to save tracklets for reid                    
                if save_tracklets and outputs[i] is not None:               
                    tracklets = sorted(tracklets, key=lambda d: d['id'])
                    with open(tracklets_data, "a", newline="") as file:
                        writer = csv.DictWriter(file, fieldnames=colnames)       
                        writer.writerows(tracklets)
...
def parse_opt():
    ...
    parser.add_argument('--save-tracklets', action="store_true", help = "flag to save tracklets for reid.")
    parser.add_argument('--compare', action="store_true", help = "compare new id with old ids")
    parser.add_argument('--use-ctl', action = "store_true", help= "use CTL Model..")
