...
class StrongSORT(object):
    ...
    def update(self, bbox_xywh, confidences, classes, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            conf = track.conf
            # bboxes.append(np.array([x1, y1, x2, y2, track_id, class_id, conf]))
            outputs['bbox'] = np.array([x1, y1, x2, y2])
            outputs['id'] = track_id
            outputs['cls'] = int(class_id)
            outputs['appearance'] = np.array(track.features)
            outputs['track'] = track
            data.append(outputs)
            outputs = {}
